...
introduce confounding changes alongside the new data. See
[[ava-dataset-integration]] in project memory for why this is a from-scratch
run rather than fine-tuning (explicit user correction), and for the AVA
subset's own provenance/decisions.
...
Per-epoch metrics -> logs/all_windowed_train_metrics.csv (gitignored).
...
from config import (
    ...
    ALL_WINDOWED_LOSS_CURVE_FILENAME,
    ALL_WINDOWED_TRAIN_LOG_FILENAME,
    ALL_WINDOWED_TRAIN_METRICS_FILENAME,
    ALL_WINDOWED_TRAINING_REPORT_FILENAME,
    BALANCED_BATCHES,
    CHECKPOINTS_DIR,
...
logger = logging.getLogger("train_windowed_all_combined")
...
def main() -> None:
    ...
    reset_dir(LOGS_DIR)
    setup_logging(LOGS_DIR / ALL_WINDOWED_TRAIN_LOG_FILENAME)
    ...
    logger.info("data sources: UniTalk-ASD + AVA-ActiveSpeaker + WASD (combined, from scratch)")

    train_loader = get_windowed_dataloader("train", extra_sources=[ava_source("train"), wasd_source("train")])
    val_loader = get_windowed_dataloader("val", extra_sources=[ava_source("val"), wasd_source("val")])
    ...
    best_checkpoint_path = CHECKPOINTS_DIR / ALL_WINDOWED_BEST_CHECKPOINT_FILENAME
    last_checkpoint_path = CHECKPOINTS_DIR / ALL_WINDOWED_LAST_CHECKPOINT_FILENAME
    metrics_path = LOGS_DIR / ALL_WINDOWED_TRAIN_METRICS_FILENAME
    ...
    logger.info(
        f"wrote {LOGS_DIR / ALL_WINDOWED_LOSS_CURVE_FILENAME} and "
        f"{LOGS_DIR / ALL_WINDOWED_ACCURACY_CURVE_FILENAME}"
    )

    report = build_training_report(
        metrics_df, best_epoch, EARLY_STOPPING_PATIENCE,
        loss_curve_filename=ALL_WINDOWED_LOSS_CURVE_FILENAME,
        accuracy_curve_filename=ALL_WINDOWED_ACCURACY_CURVE_FILENAME,
    )
    report_path = LOGS_DIR / ALL_WINDOWED_TRAINING_REPORT_FILENAME
