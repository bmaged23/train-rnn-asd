"""Evaluates a scripts/train_windowed_all_combined.py checkpoint on the
...
identical to scripts/evaluate_windowed_combined.py (full binary-classification
...
Usage:
    python scripts/evaluate_windowed_all_combined.py [--checkpoint best|last]
"""
...
SPLIT = "test"  # hardcoded — same rationale as scripts/evaluate.py
_CHECKPOINT_CHOICES = {
...
def main(checkpoint: str = "best") -> dict:
    ...
    if not checkpoint_path.exists():
        raise FileNotFoundError(f"{checkpoint_path} not found — run scripts/train_windowed_all_combined.py first")
...
if __name__ == "__main__":
    ...
    main(checkpoint=args.checkpoint)  # split is hardcoded to "test" — same rationale as scripts/evaluate.py
