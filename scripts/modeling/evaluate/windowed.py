...
"test", same rationale as scripts/modeling/evaluate/frames.py.

Unlike scripts/modeling/evaluate/frames.py, there's no frame-level masking here — each
...
This is a standalone, manually-run check — scripts/modeling/train/windowed.py does
...
Usage:
    python scripts/modeling/evaluate/windowed.py [--checkpoint best|last]
"""
...
def collect_predictions(
    ...
    (no masking needed, unlike scripts/modeling/evaluate/frames.py's per-frame version).
    """
    model.eval()
...
def main(checkpoint: str = "best") -> dict:
    ...
    if not checkpoint_path.exists():
        raise FileNotFoundError(f"{checkpoint_path} not found — run scripts/modeling/train/windowed.py first")
