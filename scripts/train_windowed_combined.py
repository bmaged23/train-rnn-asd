...
(split)]) — a fresh run, NOT fine-tuning scripts/train_windowed.py's existing
...
from scripts/train_windowed.py's UniTalk-only WINDOWED_* files, so this run
...
scripts/evaluate_windowed_combined.py's job, writing to its own
...
Usage:
    python scripts/train_windowed_combined.py
"""
...
def main() -> None:
    # Same seed + determinism setup as scripts/train_windowed.py — keeping
    ...
    # scripts/evaluate_windowed_combined.py separately for the held-out check.
